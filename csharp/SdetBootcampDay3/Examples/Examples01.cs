...
namespace SdetBootcampDay3.Examples
{
    ...
    public class Examples01
    {
        ...
        [Test]
        public async Task GetRandomJoke_CheckStatusCode_ShouldBeHttpStatusCodeOK() //public async vs public void? 
        {
        ...
        public async Task GetJokeR7UfaahVfFd_CheckCFCacheStatus_ShouldBeDynamic()
        {
            ...
            string serverHeaderValue = response.Headers! //added ! to ensure code that null will not return
                .Where(x => x.Name.Equals("CF-Cache-Status")) //LINQ 
                .Select(x => x.Value.ToString())
                .First(); //changed to .First from FirstOrDefault
        ...
        public async Task GetJokeR7UfaahVfFd_CheckJoke_ShouldBeBad()
        {
            ...
            JObject responseData = JObject.Parse(response.Content!); // converting string data in JSON object to compare/parse... create as JObject, Json strcutured object

            Assert.That(responseData.SelectToken("joke")!.ToString(), Is.EqualTo(expectedJoke)); //select token method
        }
    }
}
